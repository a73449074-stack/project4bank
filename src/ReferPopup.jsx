...
export default function ReferPopup({ onClose, user }) {
  ...
  <div className="font-bold text-lg mb-2 text-center">Refer a Friend</div>
  <div className="mb-2 text-center">Share your referral link and earn rewards for inviting friends!</div>
        <div className="bg-gray-100 rounded p-2 text-center text-xs font-mono mb-2">{referralLink}</div>
  <button className="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-6 rounded-full shadow-lg transition-all duration-200 w-full" onClick={handleCopy}>{copied ? 'Copied!' : 'Copy Referral Link'}</button>
      </div>
    </div>
  );
}
